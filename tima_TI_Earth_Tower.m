...
if TwoSpot == true
    ...
    logPfuns = {@(theta)tima_ln_prior(theta,'m_min',0.4,'theta_k_max',0.75,'k_upper_min',k_series,'k_upper_max',k_parallel) @(theta)tima_logPfun_dchi2v_dt([TData.temps_to_fit(MData.fit_ind);TData.temps_to_fit_II(MData.fit_ind)],[tima_formod_subset(theta,MData.fit_ind,formod);tima_formod_subset(theta,MData.fit_ind,formod_II)],[MData.erf(TData.temps_to_fit(MData.fit_ind)); MData.erf(TData.temps_to_fit_II(MData.fit_ind))])};% a cell of function handles returning the log probality of a each outcome
else
    logPfuns = {@(theta)tima_ln_prior(theta) @(theta)tima_logPfun_dchi2v_dt(TData.temps_to_fit(MData.fit_ind),tima_formod_subset(theta,MData.fit_ind,formod),MData.erf(TData.temps_to_fit(MData.fit_ind)))};
end
%% Run emcee
% ***************
...
%%
if TwoSpot == true
    fval = tima_fval_dchi2v_dt([TData.temps_to_fit(MData.fit_ind);TData.temps_to_fit_II(MData.fit_ind)],[tima_formod_subset(RESULTS(2,:),MData.fit_ind,formod);tima_formod_subset(RESULTS(2,:),MData.fit_ind,formod_II)],...
        [MData.erf(TData.temps_to_fit(MData.fit_ind)); MData.erf(TData.temps_to_fit_II(MData.fit_ind))],MData.nvars);
else
    fval = tima_fval_dchi2v_dt(TData.temps_to_fit(MData.fit_ind),tima_formod_subset(RESULTS(2,:),MData.fit_ind,formod),MData.erf(TData.temps_to_fit(MData.fit_ind)),MData.nvars);
end

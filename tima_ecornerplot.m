function [H,theta]=tima_ecornerplot(m,varargin)
...
%
% Aslak Grinsted 2015
% Adapted by Ari Koeppel  May. 2021

if nargin==0
    close all
    m=randn(10000,4);%m(:,2)=m(:,2)+100000;
    ecornerplot_TI_Earth(m,'ks',true,'fullmatrix',false,'grid',true);
    error('test mode... ')
    return
end
...
for r=1:M
    for c=1:max(r,M*p.fullmatrix)
        %THIS LINE BELOW CONTROLS SIZE AND POSITION OF PLOTS
        H(r,c)=subaxis(M,M,c,r,'s',0.01,'mb',0.12,'mt',0.05,'ml',0.12,'mr',0.1);
        if c==r
            if p.ks
            ...
            %Add Quantiles
            hold on
            m_conf=length([0.16,0.5,0.84]);
            quant=quantile(m(:,r),[0.16,0.5,0.84]);
            theta(:,r) = quant;
            for i=1:m_conf
               line('XData',[quant(i) quant(i)],'YData',get(gca, 'ylim'),...
                    'Color','k','LineWidth',.5, 'LineStyle', '--');      
            end
            hold off
            if r == 1
                str = sprintf('%0.3f^{+%0.2g}_{-%0.2g} W/m⋅K',...
                    quant(2),abs(quant(3)-quant(2)),abs(quant(2)-quant(1)));
            elseif r == 2
                str = sprintf('%0.3f^{+%0.2g}_{-%0.2g} W/m⋅K',...
                    quant(2),abs(quant(3)-quant(2)),abs(quant(2)-quant(1))); 
            elseif r == 3
                str = sprintf('%0.3f^{+%0.2g}_{-%0.2g}',...
                    quant(2),abs(quant(3)-quant(2)),abs(quant(2)-quant(1)));
            elseif r == 4
               str = sprintf('%0.2g^{+%0.2g}_{-%0.2g}',...
                    quant(2),abs(quant(3)-quant(2)),abs(quant(2)-quant(1)));
            elseif r == 5
                str = sprintf('%0.2g^{+%0.2g}_{-%0.2g}',...
                    quant(2),abs(quant(3)-quant(2)),abs(quant(2)-quant(1)));
            ...
            title(str,'Interpreter','tex','FontName','Myriad Pro');
            %title(p.names{r});

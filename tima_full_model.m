function [T_Surf_C] = tima_full_model(k_dry_std_upper,m,CH,CE,theta_k,theta_E,...
    ...
    windspeed_horiz,T_deep,layer_size,VWC_column,evap_depth,RH,emissivity,...
    pressure_air_pa,Observed_Temperatures,NDAYS,varargin)
%% TIMA_full
...
Subsurface_Temperatures = tima_initialize(k_dry_std_upper,rho_dry_upper,m,...
    theta_k,T_std,T_deep,Observed_Temperatures,dt,layer_size,VWC_column,RH,NDAYS,material,varargin{:});
T_Start(:) = Subsurface_Temperatures(end,end,:);
T_Start(1) = Observed_Temperatures(1);

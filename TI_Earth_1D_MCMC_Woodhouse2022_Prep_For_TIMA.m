...
for i = 1:length(fspace) %Loop to optimize layer thickness (i.e. highest resolution w/o becomming unstable or costing too much computation time)
    ...
    if isreal(Test_Result) && abs(max(Test_Result)-max(Corrected_Dry_Point_Temp)) < 50 && sum(sum(max(Subsurface_Temperatures) > (273.15+max(Corrected_Dry_Point_Temp(1:(1440/(dt/60))))))) == 0 && sum(isnan(Test_Result)) == 0 %indicators of stability
        check(i+1) = 1;
    end
...
%   Time data - struct of timeseries data variables 
      ...
      TData.evap_depth_II =evap_depth_II;

function [T_surf_C,T_sub_C,q_evap,k_eff_dt,q_conv,q_rad,q_G]= tima_heat_transfer_skin_treatment_energy_terms(k_dry_std_upper,m,CH,CE,theta_k,theta_E,...
...
T_sub_C = NaN(length(air_temp_C),NLAY); %Matrix of Temperatures for each layer and time point, T(1,t) is surface temp array that will be fit to obs data
% T = NaN(NLAY,1); %Matrix of Temperatures for each layer and time point, T(1,t) is surface temp array that will be fit to obs data
T = initial_temps;
...
for t = 1:length(air_temp_C)
    ...
    [q_evap_z(2),Soil_RH] = tima_latent_heat_model_LP1992(CE,theta_E,pressure_air_pa(t),windspeed_horiz(t),RH(t),air_temp_K(t),T(2),dug_VWC(t,1));
    ...
    while delta > GGT && t > 1 %newton-raphson iteration to solve for ground temperatures    
        %*********SENSIBLE HEAT***********
        [q_conv(t),SHCoeff] = tima_sensible_heat_model(CH,windspeed_horiz(t),air_temp_K(t),T_surf(t));
        %*******************************   
        %*********Radiative Flux***********
        if MappingMode == false %Tower mode
            if isempty(solar_azimuth_cwfromS) || isempty(solar_zenith_apparent) || isempty(aspect_cwfromS) || isempty(f_diff) && slope_angle == 0
                if isempty(albedo)
                    q_rad(t) = r_short_upper(t)-r_short_lower(t)+emissivity*r_long_upper(t)-emissivity*sigma*T_surf(t)^4;% %net heat flux entering surface assuming no transmission (Yes emissivity term in down)
                else
                    q_rad(t) = (1-albedo(t))*r_short_upper(t)+emissivity*r_long_upper(t)-emissivity*sigma*T_surf(t)^4; %net heat flux entering surface
                end
            else
                ...
                q_rad(t) = (1-albedo(t))*Incidence*(1-f_diff)*r_short_upper(t)/cosd(Z)+(1-albedo(t))*omega*f_diff*r_short_upper(t)+(1-albedo(t))*(1-omega)*r_short_lower(t)+omega*emissivity*r_long_upper(t)-omega*emissivity*sigma*T_surf(t)^4; %net heat flux entering surface
            end
        else %Mapping Mode
            phi = solar_azimuth_cwfromS(t);
            Z = solar_zenith_apparent(t);
            ...
            q_rad(t) = Lit_Fraction*q_rad_full+(1-Lit_Fraction)*((1-albedo)*omega*f_diff*r_short_upper(t)+(1-albedo)*(1-omega)*r_short_lower(t)+omega*emissivity*r_long_upper(t)-omega*emissivity*sigma*T_surf(t)^4); %net heat flux entering surface
        end
        %*******************************
        
        %*********Ground Flux***********
        %ref: Kieffer, 2013
        q_G(t) = k(1)*(T(2)-T_surf(t))/(layer_size(2)/2); %heat flux from conducting with lower layer
        %*******************************
    
        %*********Combine Heat transfer elements***********
        W = q_G(t)+q_conv(t)+q_rad(t)+q_evap_z(2); %Heat Flux, W/m^2
        ...
        delta = W/dWdt;
        q_rad(t) = q_rad(t) + 4*emissivity*sigma*T_surf(t)^3*delta; %For printing out
        q_conv(t) = q_conv(t) + SHCoeff*delta; %for printing out
        % q_evap_1 = q_evap_1 + LHCoeff*(tima_psaturationpa_Sonntag1994(T_surf(t))-tima_psaturationpa_Sonntag1994(T_surf(t-1)))/dt*delta;
        q_G(t) = q_G(t) + 2*k(1)/layer_size(2)*delta; %for printing out
        %*******************************
    ...
    for z = 2:NLAY-1 %layer loop
        if z < D_z
            if sum(layer_size(1:z)) <= evap_depth(t) %Wang 2016 inflection point
                [q_evap_z(z),Soil_RH] = tima_latent_heat_model_LP1992(CE,theta_E,pressure_air_pa(t),windspeed_horiz(t),RH(t),air_temp_K(t),T(z),dug_VWC(t,z-1));
            else
                [~,Soil_RH] = tima_latent_heat_model_LP1992(CE,theta_E,pressure_air_pa(t),windspeed_horiz(t),RH(t),air_temp_K(t),T(z),dug_VWC(t,z-1));
                q_evap_z(z) = 0; %Evap_Coeff
            end
        ...
        dT = F1*(T(z+1)+F2*T(z)+F3*T(z-1))+dt/(Cp*rho*layer_size(z))*q_evap_z(z);
        T(z) = T(z)+dT;
        if ~isempty(T_adj1)
